...
DEFINE_string(precision_mode, "allow_fp32_to_fp16", "allow_fp32_to_fp16");
DEFINE_string(op_select_impl_mode, "high_performance", "op impl mode");
...
int main(int argc, char **argv)
{
    ...
    auto decode = Decode();
    auto resize = Resize({320, 320});
    auto normalize = Normalize({103.53, 116.28, 123.675}, {57.375, 57.12, 58.395});
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        // WriteResult(all_files[i], inputs);
    }
    double average = 0.0;
    int infer_cnt = 0;
    char tmpCh[256] = {0};
