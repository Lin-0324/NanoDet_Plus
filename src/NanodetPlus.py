...
class nanodetplusWithLossCell(nn.Cell):
    def __init__(self, network, config):
        super(nanodetplusWithLossCell, self).__init__()
        ...
        self.abs = P.Abs()
        self.topk = 13
        self.batch_iter = Tensor(np.arange(0, config.batch_size * 100), mstype.int32)
    ...
    def construct(self, x, res_boxes, res_labels, res_center_priors, res_mask, nums_match):
        ...
        lt_ = self.tile(self.expand_dims(prior_center, 2), (1, 1, 100, 1)) - res_boxes[..., :2]
        rb_ = res_boxes[..., 2:] - self.tile(self.expand_dims(prior_center, 2), (1, 1, 100, 1))
        ...
        valid_mask_tilt = self.tile(self.expand_dims(valid_mask, -1), (1, 1, 100))
        pairwise_ious = self.iou(self.tile(self.expand_dims(aux_decode_bbox_pred, 2), (1, 1, 100, 1)), res_boxes) * valid_mask_tilt
        ...
        aux_cls_scores_tile = self.tile(self.expand_dims(aux_cls_scores, 2), (1, 1, 100, 1))
...
class FlattenConcat(nn.Cell):
    ...
    Args:
        config (dict): The default config of nanodetplus.
    ...
    def __init__(self, config):
        ...
        self.num_nanodetplus_boxes = config.num_nanodetplus_boxes
        self.concat = P.Concat(axis=1)
    ...
    def construct(self, inputs):
        ...
        return F.reshape(res, (batch_size, self.num_nanodetplus_boxes, -1))
...
class TrainingWrapper(nn.Cell):
    """
    Encapsulation class of nanodetplus network training.
...
class nanodetplus(nn.Cell):
    def __init__(self, backbone, config, is_training=True):
        ...
        self.D_upsample1 = P.ResizeBilinear((feature_size[1], feature_size[1]))
        ...
        self.D_upsample2 = P.ResizeBilinear((feature_size[0], feature_size[0]))
...
class nanodetplusInferWithDecoder(nn.Cell):
    """
    nanodetplus Infer wrapper to decode the bbox locations.

    Args:
        network (Cell): the origin nanodetplus infer network without bbox decoder.
        default_boxes (Tensor): the default_boxes from anchor generator
        config (dict): nanodetplus config
    Returns:
    ...
    def __init__(self, network, default_boxes, config):
        super(nanodetplusInferWithDecoder, self).__init__()
        self.network = network
...
if __name__ == "__main__":
    ...
    backbone = shufflenet(81)
    net = nanodetplus(backbone, config, True)
    out = net(x)
    print("!")
