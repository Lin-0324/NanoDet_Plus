...
def warp_boxes(boxes, M, width, height):
    ...
    if n:
        ...
        x = xy[:, [0, 2, 4, 6]]
        y = xy[:, [1, 3, 5, 7]]
        xy = np.concatenate((x.min(1), y.min(1), x.max(1), y.max(1), boxes[:, 4])).reshape(5, n).T
        # clip boxes
...
def warp_evalboxes(boxes, M, width, height):
    n = len(boxes)
    if n:
        # warp points
        xy = np.ones((n * 4, 3))
...
if __name__ == "__main__":
    ...
    img_pth = "./img.png"
    imgs = cv2.imread(img_pth)
    # box = np.array([[170,20,303,133]])
    box = np.array([[20, 170, 133, 303]])
    ...
    # print(img.shape)
    dst_shape = (320, 320)
    pipline = ShapeTransform(keep_ratio=True,perspective=0.0, scale=(0.6,1.4), stretch=((1,1),(1,1)),rotation=0,shear=0,translate=0.2,flip=0.5)
    meta = pipline(meta_data, dst_shape)
    plt.imshow(meta["img"])
