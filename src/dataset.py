...
from .box_utils import jaccard_numpy, nanodetplus_bboxes_encode
...
def preprocess_fn(img_id, image, box, is_training):
    ...
    def _infer_data(image, input_shape):
        ...
        meta = pipline(meta_data, dst_shape=input_shape)
        image = meta["img"]
        return img_id, image, np.array((img_h, img_w), np.float32), meta["warp_matrix"]

    def _data_aug(image, box, is_training, image_size=(320, 320)):
        ...
        xyxy_box = yxyx2xyxy(box)
        meta_data = dict(
            img=image,
            gt_bboxes=xyxy_box
        )
        cfg = dict(perspective=0.0, scale=(0.6, 1.4),
                   stretch=((0.8, 1.2), (0.8, 1.2)), rotation=0,
                   shear=0, translate=0.2, flip=0.5)
        pipline = ShapeTransform(keep_ratio=False, **cfg)
        meta = pipline(meta_data, dst_shape=image_size)
        image = meta["img"]
        box, label, default_boxes, mask, num_match = nanodetplus_bboxes_encode(meta["gt_bboxes"], img_id)
        return image, box, label, default_boxes, mask, num_match
...
def voc_data_to_mindrecord(mindrecord_dir, is_training, prefix="nanodetplus.mindrecord", file_num=8):
    ...
    writer.add_schema(nanodetplus_json, "nanodetplus_json")
...
def facemask_data_to_mindrecord(mindrecord_dir, is_training, prefix="nanodetplus.mindrecord0", file_num=1):
...
def data_to_mindrecord_byte_image(dataset="coco", is_training=True, prefix="nanodetplus.mindrecord", file_num=8):
...
def create_nanodetplus_dataset(mindrecord_file, batch_size, repeat_num, device_num=1, rank=0,
                               is_training=True, num_parallel_workers=8):
    """Creatr nanodetplus dataset with MindDataset."""
    ...
    if is_training:
        output_columns = ["image", "box", "label", "default_boxes","mask", "num_match"]
        trans = [color_adjust_op, normalize_op, change_swap_op]
    else:
        output_columns = ["img_id", "image", "image_shape", "warp_matrix"]
...
def create_mindrecord(dataset="coco", prefix="nanodetplus.mindrecord", is_training=True):
