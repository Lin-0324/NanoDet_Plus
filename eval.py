...
from src.NanodetPlus import nanodetplus, shufflenet, nanodetplusInferWithDecoder
from src.dataset import create_nanodetplus_dataset, data_to_mindrecord_byte_image, voc_data_to_mindrecord, \
    facemask_data_to_mindrecord
from src.box_utils import default_boxes
from src.model_utils.config import config
from src.model_utils.moxing_adapter import moxing_wrapper
from src.model_utils.device_adapter import get_device_id, get_device_num
from src.warp import warp_evalboxes
...
def nanodetplus_eval():
    context.set_context(mode=context.GRAPH_MODE, device_target=config.device_target, device_id=get_device_id())
    prefix = "nanodetplus_eval.mindrecord"
    mindrecord_dir = config.mindrecord_dir
    mindrecord_file = os.path.join(mindrecord_dir, prefix + "0")
    make_dataset_dir(mindrecord_dir, mindrecord_file, prefix)

    batch_size = 1
    ds = create_nanodetplus_dataset(mindrecord_file, batch_size=batch_size, repeat_num=1, is_training=False)
    backbone = shufflenet(81)
    net = nanodetplus(backbone, config)
    net = nanodetplusInferWithDecoder(net, Tensor(default_boxes), config)

    print("Load Checkpoint!")
    param_dict = load_checkpoint(config.checkpoint_path)
    # param_dict = load_checkpoint("./ckpt/retinanet_4-500_1.ckpt")
    ...
    for data in ds.create_dict_iterator(output_numpy=True):
        ...
        for batch_idx in range(img_np.shape[0]):
            pred_data.append({"bboxes": output[0].asnumpy()[batch_idx],
                              ...
                              "warp_matrix": M[batch_idx]})
        i += batch_size
        for sample in pred_data:
            # xyxy_bboxes = yxyx2xyxy(sample['bboxes'])
            # bboxes = xyxy_bboxes
            bboxes = sample['bboxes']
            scores = sample['scores']
            img_id = sample['img_id']
            h, w = sample['image_shape']
            M = sample['warp_matrix']
            ...
            for c in range(0, 80):
                ...
                class_bboxes = warp_evalboxes(bboxes[valid_mask], np.linalg.inv(M), w, h)
